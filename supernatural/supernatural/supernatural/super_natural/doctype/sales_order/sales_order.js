...
frappe.ui.form.on('Sales Order', {
    refresh: function (frm) {
        // Call the function on refresh to update the Outstanding Amount
        update_outstanding_amount(frm);
    },

    advance_paid: function (frm) {
        // Update Outstanding Amount when advance_paid changes
        update_outstanding_amount(frm);
    },

    installment_no: function (frm) {
        calculate_installments(frm);
    },

    installment_month: function (frm) {
        calculate_installments(frm);
    }
});

// Function to calculate and populate the installment table
function calculate_installments(frm) {
    if (frm.doc.installment_no && frm.doc.grand_total) {
        const installments = frm.doc.installment_no;
        const total_amount = frm.doc.outstanding_amount;

        // Calculate base amount per installment using Math.floor
        const base_amount = Math.floor(total_amount / installments);
        const total_floor = base_amount * (installments - 1);
        const last_amount = total_amount - total_floor;

        // Get the starting offset in months from installment_month
        const start_month_offset = frm.doc.installment_month || 0;

        // Clear the existing installment table
        frm.clear_table('installment');

        // Use staring_date as the base date, fallback to nowdate if not set
        let start_date = frm.doc.staring_date || frappe.datetime.nowdate();

        for (let i = 0; i < installments; i++) {
            let installment_date = frappe.datetime.add_months(start_date, i * start_month_offset);
            let row = frm.add_child('installment');

            row.amount = (i === installments - 1 ? last_amount : base_amount).toFixed(2);
            row.date = installment_date;
        }

        frm.refresh_field('installment');
    } else {
        frappe.msgprint(__('Please enter a valid number of installments and ensure Grand Total is available.'));
    }
}



// Function to calculate and update Outstanding Amount
function update_outstanding_amount(frm) {
    // Fetch grand_total and advance_paid
    let grandTotal = frm.doc.grand_total || 0;
    let advancePaid = frm.doc.advance_paid || 0;

    // Calculate Outstanding Amount
    let outstandingAmount = grandTotal - advancePaid;

    // Update the outstanding_amount field
    frm.set_value('outstanding_amount', outstandingAmount);
}
...
// frappe.ui.form.on('Sales Order', {
//     refresh: function (frm) {
//         // Check if the Sales Order is in the 'To Deliver and Bill' status before showing the button
//         if (frm.doc.status === 'To Deliver and Bill') {
//             // Ensure the button is only shown if no invoices are already created
//             if (!frm.doc.installment || !frm.doc.installment.some(row => row.sales_invoice_number)) {
//                 frm.add_custom_button(__('Create Sales Invoice'), function () {
//                     // Validate if the installment table has values
//                     if (frm.doc.installment && frm.doc.installment.length > 0) {
//                         let customer = frm.doc.customer;
//                         let total_invoices_created = 0; // Track the number of Sales Invoices created

//                         // Loop through each installment to create and submit a Sales Invoice
//                         frm.doc.installment.forEach(function (installment, index) {
//                             // Skip if a Sales Invoice is already linked
//                             if (installment.sales_invoice_number) {
...
//                                         customer: customer,
//                                         posting_date: installment.date, // Set posting date as installment date
//                                         due_date: installment.date, // Set due date as the installment date
//                                         edit_posting_date: true, // Allow posting date to be set
//                                         set_posting_time: true, // Allow posting time to be set
//                                         items: [
//                                             {
//                                                 item_code: "Installment Collection", // Fixed item for all invoices
//                                                 qty: 1, // Single quantity
//                                                 rate: installment.amount // Installment amount as rate
//                                             }
//                                         ],
//                                         sales_order: frm.doc.name, // Link the Sales Order
//                                         custom_sales_order_id: frm.doc.name // Set Sales Order name in custom_sales_order_id field
//                                     }
//                                 },
//                                 callback: function (response) {
//                                     let sales_invoice_id = response.message.name;

//                                     // Submit the Sales Invoice
//                                     frappe.call({
//                                         method: "frappe.client.submit",
//                                         args: {
//                                             doc: response.message // The Sales Invoice document
//                                         },
//                                         callback: function (submitResponse) {
//                                             // Store the Sales Invoice ID in the installment table
//                                             frappe.model.set_value(
//                                                 frm.doc.installment[index].doctype,
//                                                 frm.doc.installment[index].name,
//                                                 'sales_invoice_number',
//                                                 sales_invoice_id
//                                             );

//                                             // Increment the total invoices created counter
//                                             total_invoices_created++;

//                                             // Refresh the installment table
//                                             frm.refresh_field('installment');

//                                             // Check if all invoices are created
//                                             if (total_invoices_created === frm.doc.installment.length) {
//                                                 // Save the document after updating all installments
//                                                 frm.save_or_update();

//                                                 // Show success message
//                                                 frappe.msgprint(__('Sales Invoices have been successfully created and submitted.'));

//                                                 // Remove the button to prevent duplicates
...
//                         });
//                     } else {
//                         frappe.msgprint(__('No installments found. Please add installments.'));
//                     }
//                 });
//             }
//         }
...
// frappe.ui.form.on('Sales Order', {
//     cus_submit: function (frm) {
//         if (frm.doc.advance_paid > 0) {
//             // Check if a row with the same date already exists
//             const existing_entry = frm.doc.pay_invoice.find(
//                 (row) => row.date === frm.doc.posting_date
//             );

//             if (existing_entry) {
//                 frappe.msgprint(__('An entry with this date already exists in the table.'));
//                 return; // Stop execution
//             }

//             // Validation to check if a Sales Invoice is already created
//             const existing_invoice = frm.doc.pay_invoice.find(
//                 (row) => row.sales_invoice_number
//             );

//             if (existing_invoice) {
//                 frappe.msgprint(__('A Sales Invoice has already been created for this Sales Order.'));
//                 return; // Stop execution
//             }
...
//                         docstatus: 1, // Ensure only submitted entries are fetched
//                         paid_amount: frm.doc.advance_paid, // Match the advance paid amount
//                     },
//                     order_by: "creation desc", // Get the latest payment entry
//                     limit_page_length: 1,
//                 },
//                 callback: function (r) {
//                     if (r.message && r.message.length > 0) {
//                         const payment = r.message[0];
...
//                                     items: [
//                                         {
//                                             item_code: "Advance Collection",
//                                             qty: 1,
//                                             rate: payment.paid_amount,
//                                             amount: payment.paid_amount,
//                                         },
//                                     ],
//                                     is_pos: 1, // If POS is applicable
//                                     payments: [
//                                         {
...
//                                 if (res.message) {
//                                     const sales_invoice = res.message;
...
//                                                 // Update pay_invoice table
//                                                 frm.clear_table("pay_invoice");
//                                                 const row = frm.add_child("pay_invoice");
//                                                 row.date = payment.posting_date;
//                                                 row.amount = payment.paid_amount;
//                                                 row.sales_invoice_number = submit_res.message.name;

//                                                 // Refresh the field to reflect changes
//                                                 frm.refresh_field("pay_invoice");

//                                                 frappe.msgprint(
//                                                     __(
//                                                         `Sales Invoice ${submit_res.message.name} created, submitted, and added to the pay_invoice table.`
//                                                     )
//                                                 );
//                                             } else {
//                                                 frappe.msgprint(
//                                                     __('Failed to submit the Sales Invoice.')
//                                                 );
//                                             }
//                                         },
//                                     });
...
//                         });
//                     } else {
//                         frappe.msgprint(__('No Payment Entry found for the advance paid amount.'));
//                     }
//                 },
//             });
//         } else {
//             frappe.msgprint(__('Advance Paid is zero or not set.'));
//         }
//     },
// });
                                ...
                                callback: function (response) {
                                    ...
                                    frappe.call({
                                        ...
                                        callback: function (submitResponse) {
                                            ...
                                            // Handle advance paid logic
                                            if (frm.doc.advance_paid > 0 && total_invoices_created === frm.doc.installment.length) {
                                                frappe.call({
                                                    method: "frappe.client.get_list",
                                                    args: {
                                                        doctype: "Payment Entry",
                                                    ...
                                                    callback: function (r) {
                                                        if (r.message && r.message.length > 0) {
                                                            ...
                                                            // Create and Submit Sales Invoice for Advance Paid
                                                            frappe.call({
                                                                method: "frappe.client.insert",
                                                                args: {
                                                                    doc: {
                                                                        ...
                                                                        frappe.call({
                                                                            ...
                                                                            callback: function (submit_res) {
                                                                                if (submit_res.message) {
                                                                                    ...
                                                                                    // Refresh the field and save the document
                                                                                    frm.refresh_field("pay_invoice");
                                                                                    frm.save_or_update(); // Automatically save the document
...
// frappe.ui.form.on("Sales Order", {
//     refresh: function(frm) {
//         if (frm.doc.docstatus === 1) {  // Ensure Sales Order is submitted
...
//                     callback: function(r) {
//                         if (r.message) {
//                             // Sync the Maintenance Schedule document
//                             let doc = frappe.model.sync(r.message)[0];

//                             // Open the Maintenance Schedule form
//                             frappe.set_route("Form", "Maintenance Schedule", doc.name);

//                             // Wait for the form to load, then set the customer field
...
//                             });
//                         } else {
//                             console.error("❌ No Maintenance Schedule returned from server!");
//                         }
//                     }
//                 });
